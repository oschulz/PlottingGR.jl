...
const GRInterval = Interval{:closed,:closed,Float64}


function _gr_setup_plot_2d(;
    x_limits::GRInterval = 0.0..1.0,
    y_limits::GRInterval = 0.0..1.0,
    x_tick::Float64 = (maximum(x_limits) - minimum(x_limits)) / 10,
    y_tick::Float64 = (maximum(y_limits) - minimum(y_limits)) / 10,
    major_xtick::Int = 2,
    major_ytick::Int = 2,
    ...
    x_min = minimum(x_limits)
    x_max = maximum(x_limits)
    y_min = minimum(y_limits)
    y_max = maximum(y_limits)
